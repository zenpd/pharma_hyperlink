...
class TestCtdLeafPath:
    ...
    @pytest.mark.parametrize("text", ["BMI in kg/m2", "dose of 5 mg/m2", "bare m2 token"])
    def test_unit_strings_no_longer_match(self, text: str) -> None:
        ...
        # now requires an actual leaf path.
        reg = default_registry()
...
class TestVisitRef:
    ...
    def test_matches_visit_references(self, text: str, unit: str, n: str) -> None:
        reg = default_registry()
    ...
    def test_rejects_bare_timepoints(self, text: str) -> None:
        # The literal word "Visit" is required — bare timepoints are prose, not
        # cross-references, and there are ~5x as many of them.
        reg = default_registry()
