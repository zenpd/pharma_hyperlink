...
from hyperlink_engine.workers.celery_app import reset_app
from hyperlink_engine.workers.tasks import _registered_tasks
...
def test_output_paths_for_uses_relative_layout(tmp_path: Path) -> None:
    ...
    assert output.name == "d_linked.docx"
...
def test_output_paths_for_shallow_path_falls_back_to_name(tmp_path: Path) -> None:
    ...
    output, report = _output_paths_for(source, desc)
    assert output.name == "shallow_linked.docx"
