...
class RunLocation(BaseModel):
    ...
    # Whole-reference (compound) linking: when ``end_run_index`` is set and is
    # greater than ``run_index``, the span runs from (run_index, char_start)
    # through (end_run_index, end_char) and is wrapped as ONE hyperlink across
    # those runs. Default None preserves the single-run behavior used everywhere
    # else (the link is wrapped inside ``run_index`` from char_start to char_end).
    end_run_index: int | None = Field(default=None, ge=0)
