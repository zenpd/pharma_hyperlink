...
def _build_llm_resolver() -> Any | None:
    """Build the LLM tie-breaker once per resolve pass, only when enabled.

    Returns an ``LlmDisambiguator`` (used here for its ``resolve_target`` method)
    or ``None`` when ``llm_resolve_ambiguous`` is off or the LLM stack can't be
    constructed — in which case resolution stays purely deterministic.
    """
    try:
        from hyperlink_engine.config.settings import get_settings

        if not get_settings().llm_resolve_ambiguous:
            return None
        from hyperlink_engine.core.detection.llm_disambiguator import build_disambiguator

        return build_disambiguator()
    except Exception as exc:  # noqa: BLE001 — never break resolution to add a tie-breaker
        _log.warning("llm_resolver_unavailable", error=str(exc))
        return None


def _pick_sibling_or_llm(
    cands: list[dict[str, Any]],
    source_path: str,
    det: dict[str, Any],
    resolver: Any | None,
) -> str | None:
    """Deterministic ``_pick_sibling`` first; only when it cannot separate ≥2
    candidates (returns None) AND the LLM tie-breaker is enabled, ask the LLM
    ("resolve_v1") to choose one using the reference + its surrounding context.

    Additive by construction: a deterministic pick is returned unchanged, so the
    LLM can only RECOVER a link that would otherwise be dropped — it never
    overrides an already-resolved target. Never raises."""
    picked = _pick_sibling(cands, source_path)
    if picked is not None or resolver is None or len(cands) < 2:
        return picked
    id_to_path: dict[str, str] = {}
    items: list[tuple[str, str]] = []
    for i, c in enumerate(cands):
        cid = f"c{i}"
        id_to_path[cid] = c["path"]
        items.append((cid, f'{c.get("doc_type", "doc")}: {Path(c["path"]).stem}'))
    try:
        chosen = resolver.resolve_target(
            ref_text=det.get("text", ""),
            context=det.get("context", ""),
            candidates=items,
        )
    except Exception as exc:  # noqa: BLE001 — tie-breaker must never break resolution
        _log.warning("llm_resolve_failed", error=str(exc))
        return None
    target = id_to_path.get(chosen or "")
    if target:
        det["llm_resolved"] = True  # provenance: this link came from resolve_v1
    return target
...
def _resolve_one(
    ...
    if label in ("DOC_REF", "DOC_ID"):
        ...
        if dt:
            ...
            return _pick_sibling_or_llm(typed, source_path, det, resolver)
        if label == "DOC_REF":
    ...
    if label in _STRUCT_LABELS:
        ...
        if qdt:
            ...
            picked = _pick_sibling_or_llm(typed, source_path, det, resolver)
            if picked:
                return picked
...
def _pdf_page_count(path: Path) -> int | None:
    """Page count of a target PDF, or None if it is not a readable PDF.

    PDF internal/cross-doc links are page-level **GoTo** jumps (not docx-style named
    bookmarks), so the right check is "does the link's target page exist in the
    file?" — this lets valid PDF links validate ``ok`` instead of the old blanket
    ``unverified``. Best-effort: any failure degrades to ``None`` → ``unverified``,
...
def node_validate(state: PipelineState) -> PipelineState:
    ...
    * internal bookmark  → the anchor exists in this document's own linked output
    ...
    _pg_cache: dict[str, int | None] = {}

    def _pages(path: Path) -> int | None:
        key = str(path)
        if key not in _pg_cache:
            _pg_cache[key] = _pdf_page_count(path)
        return _pg_cache[key]

    def _validate_pdf(path: Path, p: dict[str, Any]) -> tuple[str, str | None]:
        """A PDF target (the file is already known to exist): its links are
        page-level GoTo jumps, so verify the link's target page is inside the file
        rather than hunting for a docx bookmark. Worst case is ``unverified`` —
        never a false ``broken``."""
        n = _pages(path)
        if n is None:
            return "unverified", "PDF not readable for page check"
        tp = p.get("target_page")
        if tp is None:
            return "ok", None  # opens the PDF at page 1 — a valid landing
        try:
            tp_i = int(tp)
        except (TypeError, ValueError):
            return "unverified", "PDF target page not numeric"
        if 1 <= tp_i <= n:
            return "ok", None
        return "unverified", f"PDF target page {tp_i} outside 1..{n}"
    ...
    def _validate(p: dict[str, Any], link_kind: str, target: str) -> tuple[str, str | None]:
        ...
        if link_kind == "cross_doc":
            ...
            if tgt.suffix.lower() == ".pdf":
                return _validate_pdf(tgt, p)  # PDF: page-level GoTo, verify the page
            bms = _bookmarks(tgt)
            if bms is None:
        ...
        if tpath.suffix.lower() == ".pdf":
            return _validate_pdf(tpath, p)  # PDF: page-level GoTo, verify the page
