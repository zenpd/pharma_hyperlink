...
def _add_pdf_references(index: dict[str, dict[str, Any]], source_path: str | Path) -> None:
    ...
    doc = fitz.open(str(source_path))
    try:
        lines: list[tuple[int, str, list | None]] = []
        for page_index in range(doc.page_count):
            page = doc.load_page(page_index)
            for block in page.get_text("dict").get("blocks", []):
                if block.get("type", 0) != 0:
                ...
                for line in block.get("lines", []):
                    ...
                    lines.append((page_index, text, [float(v) for v in bb] if bb else None))
    finally:
        doc.close()

    start = next((i for i, (_p, t, _b) in enumerate(lines) if is_references_heading(t)), None)
    if start is None:
        return
    index.setdefault(REF_HEADING_KEY, {"page_index": lines[start][0], "bbox": lines[start][2]})
    for page_index, text, bbox in lines[start + 1 :]:
        if is_next_section_after_refs(text):
            break
        key = parse_ref_entry_key(text)
        if key and key not in index:
