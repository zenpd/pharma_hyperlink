 ...
 * occurrence of the citation (which is the citation itself).
 *
...
export function findRefEntryIndex(blocks: RefBlock[], anchor?: string): number {
  ...
  if (ay) {
    const [, surname, year] = ay;
    const hit = blocks.find((b) => {
      const t = (b.text || "").trim().toLowerCase();
      return t.startsWith(surname) && t.includes(year);
    });
  ...
  if (num) {
    ...
    const hit = blocks.find((b) => re.test((b.text || "").trim()));
    return hit ? hit.index : -1;
  }
  return -1;
}
