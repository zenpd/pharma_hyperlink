...
from hyperlink_engine.pipeline.tasks import DocPipelineResult, process_document
...
class DossierBatchDescriptor:
    ...
    def from_directory(
        ...
        *,
        ...
        sources = sorted(Path(source_root).rglob(glob.split("**/")[-1]))
        return cls(
            sources=sources,
...
def _output_paths_for(source: Path, batch: DossierBatchDescriptor) -> tuple[Path, Path]:
    ...
    output = batch.output_root / rel.with_suffix(".linked.docx")
...
def _run_celery(
    ...
    from hyperlink_engine.pipeline.tasks import register_celery_tasks  # local import
