...
class ExtractorConfig:
    ...
    @classmethod
    def full_cascade(cls, *, prefer_stub: bool = True) -> "ExtractorConfig":
        return cls(with_ner=True, with_llm=True, prefer_llm_stub=prefer_stub)
...
def get_extractor(config: ExtractorConfig | None = None) -> EntityExtractor:
    ...
    with _extractor_lock:
        ...
        if cached is not None:
            return cached
        if cfg.with_llm:
            extractor = full_cascade(prefer_stub=cfg.prefer_llm_stub)
        elif cfg.with_ner:
